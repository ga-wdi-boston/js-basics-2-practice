...
// converts variable calling each array of lines (start and end) to string to pass test
  let startLineArray;
    if (startLine === 'Red') {
      startLineArray = redLine;
  } else if (startLine === 'Green'){
      startLineArray = greenLine;
  } else {
    startLineArray = orangeLine;
  }

  let endLineArray;
    if (endLine === 'Red') {
      endLineArray = redLine;
  } else if (endLine === 'Green'){
      endLineArray = greenLine;
  } else {
    endLineArray = orangeLine;
  }


  if (startLine === endLine) {
    return countStopsSameLine(startLineArray, startStation, endStation);
  } else {
    return (countStopsToPark(startLineArray, startStation) + countStopsToPark(endLineArray, endStation));
  }
};
