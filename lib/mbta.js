...
/*
// number of stops are stored as keys of a dictionary
  let numberOfStopsTaken = {};
  for (let i = 0; i < stopsBetweenStations.length; i++) {
    let stop = stopsBetweenStations[i];
    numberOfStopsTaken[stop] = true;
}

let numberofStops = [];
let k = 0;
for(numberofStops[k++] in numberOfStopsTaken); // jshint ignore:line
...
const addOne = function(num){
  ...
  for (let i = 0; i < stopsBetweenStations.length; i++) {
    let stop = stopsBetweenStations[i];
    stopFrequencies[word] = addOne(stopFrequencies[word]);
  }
};
*/
