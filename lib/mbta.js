...
const stopsBetweenStations = (startLine, startStation, endLine, endStation) => {
  if (startLine === endLine) {
  	// calculation
    let startStationIndex = mbta[startLine].indexOf(startStation);
    let endStationIndex = mbta[endLine].indexOf(endStation);
    let numStops = Math.abs(startStationIndex - endStationIndex);
    return numStops;
  } else {
...
const stopsBetweenStations = (startLine, startStation, endLine, endStation) => { if (startLine === endLine) {return Math.abs((mbta[startLine].indexOf(startStation)) - (mbta[endLine].indexOf(endStation)));} else { return (Math.abs((mbta[startLine].indexOf('Park Street'))-(mbta[startLine].indexOf(startStation)))) + (Math.abs((mbta[endLine].indexOf('Park Street'))-(mbta[endLine].indexOf(endStation))))}};
*/
