...
let parkToEnd;
let numberOfStops;
let startToHaymarket;
let haymarketToEnd;

//All line stops in arrays
redLineStops = ["South Station", "Park Street", "Kendall", "Central", "Harvard", "Porter", "Davis", "Alewife"];
greenLineStops = ["Haymarket","Government Center", "Park Street", "Bolyston", "Arlington", "Copley", "Hynes", "Kenmore"];
...
const stopsBetweenStations = function(startLine, startStation, endLine, endStation) {
  ...
  if (startLine === endLine && startStation !== endStation){
    ...
    /*This part should be rewritten to stringmatch and find the intersecting
    station stop rather than hardcoding Park Street and Haymarket as intersection
    stations */
    ...
    startToHaymarket = Math.abs(startIndex - mbta[startLine].indexOf("Haymarket"));
    haymarketToEnd = Math.abs(mbta[endLine].indexOf("Haymarket")-mbta[endLine].indexOf(endStation));
    numberOfStops = startToHaymarket + haymarketToEnd;
    return numberOfStops
  } else {
    console.log('Hmmm, something is wrong with your journey');
  }
